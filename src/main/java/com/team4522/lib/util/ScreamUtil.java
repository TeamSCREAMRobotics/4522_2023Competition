...
import frc2023.Constants.SwerveAutoConfig;
...
public class ScreamUtil {
	...
	public static TrajectoryConstraint createTrajectoryConstraint(SwerveAutoConfig config){
		return new TrajectoryConstraint(){

			@Override
			public double getMaxVelocityMetersPerSecond(Pose2d poseMeters, double curvatureRadPerMeter,
					double velocityMetersPerSecond) {
				return config.kAutoMaxSpeed;
			}
	
			@Override
			public MinMax getMinMaxAccelerationMetersPerSecondSq(Pose2d poseMeters, double curvatureRadPerMeter,
					double velocityMetersPerSecond) {
				return new MinMax(config.kAutoMaxDeceleration, config.kAutoMaxAcceleration);
			}
		};
	}	
}
