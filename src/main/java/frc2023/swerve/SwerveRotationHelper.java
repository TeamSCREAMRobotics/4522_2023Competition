...
public class SwerveRotationHelper {
	...
	private Timer openLoopTimer;
	private PIDController mSnapController;
	private PIDController mHoldController;
	...
	public SwerveRotationHelper(){
		mSnapController = ScreamUtil.createPIDController(SwerveConstants.snapRotationPIDConstants, 0.2);
		mSnapController.enableContinuousInput(-Math.PI, Math.PI);

		mHoldController = ScreamUtil.createPIDController(SwerveConstants.holdRotationPIDConstants, 0.2);
	 ...
	 * @param rotationInput the rotation input is only for open loop control. If using snap or hold, make it 0.0
	 */
	public double calculateRotation(Rotation2d measured, double rotationInput){
		switch(mode){
