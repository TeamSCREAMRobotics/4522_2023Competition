...
public class SwerveSetpointGenerator {
    ...
    private double findRoot(Function2d func, double x_0, double y_0, double f_0, double x_1, double y_1, double f_1, int iterations_left) {
        if (iterations_left < 0 || ScreamUtil.epsilonEquals(f_0, f_1)) {
            return 1.0;
        }
    ...
    public SwerveSetpoint generateSetpoint(final KinematicLimits limits, final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredState, double dt) {
        ...
        if (ScreamUtil.epsilonEquals(ScreamUtil.chassisSpeedsToTwist2d(desiredState), new Twist2d())) {
            need_to_steer = false;
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            if (prevSetpoint.moduleStates[i].speedMetersPerSecond < 0.0) {
                prev_heading[i] = prev_heading[i].plus(Rotation2d.fromDegrees(180));
            }
            ...
            if (desiredModuleState[i].speedMetersPerSecond < 0.0) {
                desired_heading[i] = desired_heading[i].plus(Rotation2d.fromDegrees(180));
            }
            if (all_modules_should_flip) {
                double required_rotation_rad = Math.abs(prev_heading[i].times(-1).rotateBy(desired_heading[i]).getRadians());
        ...
        ScreamUtil.epsilonEquals(ScreamUtil.chassisSpeedsToTwist2d(prevSetpoint.chassisSpeeds), new Twist2d())
            && !ScreamUtil.epsilonEquals(ScreamUtil.chassisSpeedsToTwist2d(desiredState), new Twist2d())) {
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            if (ScreamUtil.epsilonEquals(prevSetpoint.moduleStates[i].speedMetersPerSecond, 0.0)) {
                ...
                if (ScreamUtil.epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
                    // Goal angle doesn't matter. Just leave module at its current angle.
                    overrideSteering.set(i, Optional.of(prevSetpoint.moduleStates[i].angle));
                    continue;
                }

                var necessaryRotation = prevSetpoint.moduleStates[i].angle.times(-1).rotateBy(
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            if (maybeOverride.isPresent()) {
                ...
                if (flipHeading(retStates[i].angle.times(-1).rotateBy(override))) {
                    retStates[i].speedMetersPerSecond *= -1.0;
                }
                retStates[i].angle = override;
            }
            final var deltaRotation = prevSetpoint.moduleStates[i].angle.times(-1).rotateBy(retStates[i].angle);
            if (flipHeading(deltaRotation)) {
                retStates[i].angle = retStates[i].angle.plus(Rotation2d.fromDegrees(180));
                retStates[i].speedMetersPerSecond *= -1.0;
            }
        }
